...
int expandLzg(const unsigned char* input, int inputSize,
	...
	/* ESP32 port modification: this 65500-byte work buffer was originally a
	   stack array; moved to the heap so it does not overflow the (small)
	   FreeRTOS task stack. See README (Reused third-party code). */
	unsigned char          *output=(unsigned char*)malloc(65500);
	register int           loc;
	register unsigned char rep, k;

	if (output==NULL) return COMPRESS_RESULT_FATAL;
	...
	*output2=(unsigned char*)malloc(*outputSize);
	...
	if (oCursor>=(*outputSize)) { free(output); return inputSize; } /* TODO: check if this case never happens !!! */
